...
# KEEP ALL ABOVE HERE
# Below is EDA that can be moved to graphs
...
mod <- lm(relative_sqrtEPA ~ max_EIR_atMaxChange + frames_from_start_atMaxChange, data = maxChangesEIR)
summary(mod)
plot(mod)

mod <- lm(expectedPointsAdded ~ max_EIR_atMaxChange + frames_from_start_atMaxChange, data = maxChangesEIR)
summary(mod)
plot(mod)
...
ggplot(quickest_set_plot, aes(x = bucket_angle_min, y = 1, fill = expected_EIR)) +
  geom_col(color = "black") +
  coord_polar(start = 66) +
  scale_x_continuous(limits = c(-180,180)) +
  scale_y_continuous(limits = c(0,1)) +
  scale_fill_gradient2(high = "darkgreen", low = "deeppink", midpoint = mean(quickest_set_plot$expected_EIR, na.rm = TRUE),
                       limits = c(25,100)) +
  labs(title = "EIR's effect on Greatest Total Change of Direction Over Time", 
       x = "Relative Total Change", y = "", 
       fill = "Expected EIR") +
  theme_minimal() +
  theme(axis.text.y = element_blank(),
        ...
        plot.subtitle = element_text(size = 12),
        legend.key.size = unit(1, "cm")) +
  transition_states(seconds_from_start_atMaxChange, transition_length = 0, state_length = 2) +
  enter_fade() +
  exit_fade() +
  ease_aes('linear') +
  # display the current frameId
  labs(subtitle = "Seconds from handoff: {closest_state}")
```
