...
```{r}
maxChangesEIR %>%
  select(frames_from_start_atMaxChange, expectedPointsAdded) %>%
  mutate(bucket = cut(frames_from_start_atMaxChange, breaks = seq(min(frames_from_start_atMaxChange), max(frames_from_start_atMaxChange), 3))) %>%
  group_by(bucket) %>%
  summarise(n = n(),
            meanEPA = mean(expectedPointsAdded, na.rm = TRUE)
         ) %>%
  ggplot(aes(x = as.factor(frames_from_start_atMaxChange), y = n, fill = meanEPA)) +
  geom_col()
