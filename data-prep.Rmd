...
Run for each week of tracking data, then run the combining code chunks at the end.
Saves each week of tracking data as 'data/wkX_processed_data.csv'
Must run for each week first due to R storage capacity.
...
To run: Change file path on line below. Run everything before last code 
chunk then run last code chunk to save combined data to your data directory.

```{r}
tracking_file <- "data/tracking_week_1.csv"
```

# Initial data cleaning pass: Filtering for Zone Run Plays inside the tackle box
...
# Finding Total Directional Change and Relative Directional Change
...
SameDirection_TrackData <- SameDirection_TrackData %>%
  group_by(gamePlayId) %>%
  mutate(y_at_5 = ifelse(frameId == 5, y, NA)) %>%
...
check_intersection <- function(frameId, ballCarrierStartX, ballCarrierStartY, ballCarrierEndX, ballCarrierEndY, players_df) {
  ...
  players_df$rank <- rank(players_df$y_at_5)
...
```{r}
SameDirection_TrackData <- merge(SameDirection_TrackData, splitGap, by = "gamePlayId") %>% select(-y_at_5)
```


```{r}
# Just the first of tackle, first_contact, touchdown, out of bounds or gap intersection that occurs
triangle_end_point <- SameDirection_TrackData %>%
  mutate(
    ol_x = ifelse(position %in% c("T", "G", "C"), x, NA),
    bc_x = ifelse(ballCarrierId == nflId, x, NA)
  ) %>%
  group_by(gamePlayId) %>%
  filter(event %in% c("tackle", "first_contact", "touchdown", "out_of_bounds") |
    frameId == frameId_intersect |
      # bc_x is greater than all ol_x
      (bc_x > max(ol_x, na.rm = TRUE) 
      )) %>%
...
# Find angle of locational change
The angle we want is the inside angle at the point of locational change.
...
# Finding the greatest directional change for the play
...
angls <- SameDirection_TrackData %>%
  ...
  reframe(gamePlayId, frameId,
    ...
    orientation_change = smallest_angle_diff(o, lead(o, 1, default = NA)),
    ...
    relative_orientation_change = smallest_angle_diff(relative_orientation, lead(relative_orientation, 1, default = NA)),
...
# Where the greatest directional change occurs calculate that angle with the end point
...
# Save resulting data as all_processed_data.csv and filtered_processed_data.csv

```{r}
# Save dataframe as CSV
write.csv(SameDirection_TrackData, 
          file = "data/wk2_processed_data.csv", row.names = FALSE)
```

Save compiled data frame with a few filtering steps because of space issues.
...
write.csv(combined_data, 
          file = "data/all_processed_data.csv", row.names = FALSE)

data <- combined_data %>% 
  filter(frameId == frameId_at_max_change)

write.csv(data, 
          file = "data/filtered_processed_data.csv", row.names = FALSE)
```


```{r}
write.csv(data, 
          file = "edge-setting-epa/filtered_processed_data.csv", row.names = FALSE)
```
