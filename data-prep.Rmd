...
Run to get triangle/angle of change data for all weeks.
...
# Calculate the points of the path triangle

Make gamePlayId which is a combination of gameId and playId. This helps us 
identify each unique play.

```{r}
SameDirection_TrackData$gamePlayId <- paste(SameDirection_TrackData$gameId,
                                            SameDirection_TrackData$playId)
```
...
SameDirection_TrackData <- merge(SameDirection_TrackData, start, 
                                 by = "gamePlayId")
```
...
```{r}
SameDirection_TrackData <- merge(SameDirection_TrackData, splitGap, 
                                 by = "gamePlayId")
```
...
SameDirection_TrackData <- merge(SameDirection_TrackData, triangle_end_point, 
                                 by = "gamePlayId")
```

Angle of greatest change point
...
max_angle_change <- SameDirection_TrackData %>%
  group_by(gamePlayId) %>%
...
```{r}
SameDirection_TrackData <- merge(SameDirection_TrackData, max_angle_change %>% select(-playSide), by = "gamePlayId")
```

Where the greatest change in direction occurs, calculate that angle with the 
end point
...
```{r}
SameDirection_TrackData <- merge(SameDirection_TrackData, full_angle 
                                 %>% select(gamePlayId, angle_greatestChange), 
                                 by = "gamePlayId")
```
...
For playside left:
Player who is closest to ball carrier with a larger y coordinate at 
greatest rate of change.
For playside right:
Player who is closest to ball carrier with a smaller y coordinate at 
greatest rate of change.

```{r eval=FALSE, include=FALSE}
SameDirection_TrackData %>%
...
write.csv(SameDirection_TrackData, 
          file = "data/wk1_processed_data.csv", row.names = FALSE)
```
