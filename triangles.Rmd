...
```{r}
test_run <- wk2_track_data %>% filter(gameId == 2022091900, playId == 3980)
nfl_field(c(0, 120)) +
  geom_point(
...
testing_run <- test_run %>% group_by(frameId)  %>%
  distinct(frameId, team_EIR, total_change) %>% filter(team_EIR == max(team_EIR, na.rm = TRUE)) %>%
...
ggplot(testing_run, 
       aes(x = frameId, y = team_EIR)) +
  geom_line() +
  geom_line(aes(x = frameId, y = total_change), color = "red") +
...
ggplot(plotdf, aes(x = bucket_angle_mean, y = 1)) +
  ...
  scale_y_continuous(limits = c(0,1)) +
  labs(title = "EPA by Relative Angle", x = "Relative Angle", y = " ", subtitle = "Values < 0 are Cutbacks, Values > 0 are Bounces",
       fill = "EPA")  +
  scale_fill_gradient2(low = "darkblue", high = "firebrick4", midpoint = mean(plotdf$meanEPA)) +
  theme_minimal() +
  theme(axis.text.y = element_blank(),
