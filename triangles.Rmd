...
check_intersection <- function(ballCarrierStartX, ballCarrierStartY, ballCarrierEndX, ballCarrierEndY, players_df) {
  # Rank players by their y-coordinate
  ...
  any(sapply(player_pairs, function(pair) {
    ...
    # intuition: only check intersections next to each other by position
    # Based on the y coordinate, rank all offensive linemen from top to bottom
    # If the difference in rank is 1, then the players are next to each other
    # Return FALSE if the players are not next to each other
    
    # Check if the players are nect to each other based on their y-coordinate rank
    if (abs(player1$rank - player2$rank) == 1) {
      # If players are next to each other, check for line segment intersection
      is_line_segment_intersect(ballCarrierStartX, ballCarrierStartY, ballCarrierEndX, ballCarrierEndY,
