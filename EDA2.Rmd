...
nfl_field(c(0, 120)) +
  geom_point(
    data = test_run %>% filter(frameId >= 18 & frameId <= 37), aes(x = x, y = y, color = club),
    show.legend = FALSE,
    size = 2
  ) +
  geom_point(data = test_run %>% filter(nflId == 46144), aes(x = x, y = y), color = "black") +
  geom_point(data = test_run %>% filter(nflId == 52585), aes(x = x, y = y), color = "black") +
