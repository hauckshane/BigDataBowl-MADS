...
eir_df_left %>%
  ...
  tab_header(
    ...
    fn = scales::col_numeric(
      palette = c("lightcyan", "darkcyan"),
      domain = NULL
    )) %>% data_color(
    columns = c("Avg EPA"),
    fn = scales::col_numeric(
      palette = c("darkcyan", "white", "firebrick4"),
      domain = NULL
    ))
...
eir_df_all <- eir_df_all[order(-eir_df_all$meanEIR), ]
eir_df_all <- eir_df_all[order(-eir_df_all$freqEIR), ]

# Make pretty column names
names(eir_df_all) <- c("Team", "Name", "Playside", "Num. Plays",
                   "Num. Plays for Player", "prop", "Avg Time",
                   "Avg EIR", "Avg EPA", "Setting Freq", "Weighted EIR", "Percentile")

drops <- c("Playside")
eir_df_all <- eir_df_all[ , !(names(eir_df_all) %in% drops)]
