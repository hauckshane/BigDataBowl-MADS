...
```{r}
eir_df <- read.csv("edge-setting-epa/eir_epa.csv")
```
...
eir_df_left %>%
  head(10) %>%
  select('Team', 'Name', 'Setting Freq', 'Avg Time', 'Avg EIR', 'Avg EPA') %>%
  gt() %>%
  fmt_number(
    columns = c("Avg EPA"), 
    decimals = 2
...
# 2022100300 3793
#2022100300 1942
animation_data <- SFLA_full %>% filter(gamePlayId == "2022100300 1942")
...
p3 <- ggplot(animating_graphs, aes(x = frameId - first(animation_data$start_frameId))) +
  geom_line(aes(y = scale_total_change, color = "Ball Carrier\nDirection Change"), alpha = 0.1, lwd = 1.5) +
  geom_line(aes(y = scale_s, color = "Ball Carrier Speed"), alpha = 0.1, lwd = 1.5) +
  geom_line(aes(y = scale_a, color = "Ball Carrier Acceleration"), alpha = 0.1, lwd = 1.5) +
  geom_line(aes(y = scale_dist, color = "Distance to\nBall Carrier"), alpha = 0.1, lwd = 1.5) +
  geom_line(aes(y = scale_speed_towards_bc, color = "Speed Towards\nBall Carrier"), alpha = 0.1, lwd = 1.5) +
  geom_line(aes(y = scale_acceleration_towards_bc, color = "Acceleration Towards\nBall Carrier"), alpha = 0.1, lwd = 1.5) +
  geom_line(aes(y = scale_o_to_bc, color = "Orientation to\nBall Carrier"), alpha = 0.1, lwd = 1.5) +
  geom_line(aes(y = scale_angle_to_path, color = "Angle to Path"), alpha = 0.1, lwd = 1.5) +
  geom_line(aes(y = scale_time, color = "Time"), alpha = 0.1, lwd = 1.5) +
  geom_line(data = animating_graphs %>% group_by(frameId) %>%
              filter(edge_intensity_rating_no_time == max(edge_intensity_rating_no_time, na.rm = TRUE)),
            aes(y = edge_intensity_rating_no_time, color = "Edge Intensity\nRating"), linewidth = 3) +
  transition_reveal(frameId) +
  ...
  scale_x_continuous(breaks = seq(first(animation_data$start_frameId)+1 - first(animation_data$start_frameId),
                     ...
                     limits = c(first(animation_data$start_frameId)+1 - first(animation_data$start_frameId),
                                first(animation_data$frameId_at_max_change) - first(animation_data$start_frameId))) +
  scale_color_manual(values = c("Edge Intensity\nRating" = "gold", 
                                "Ball Carrier\nDirection Change" = "firebrick1",
                                "Ball Carrier Speed" = "firebrick3",
                                "Ball Carrier Acceleration" = "firebrick4",
                                "Distance to\nBall Carrier" = "dodgerblue",
                                "Speed Towards\nBall Carrier" = "dodgerblue2",
                                "Acceleration Towards\nBall Carrier" = "dodgerblue3",
                                "Orientation to\nBall Carrier" = "dodgerblue4",
                                "Angle to Path" = "dodgerblue1",
                                "Time" = "limegreen"),
                     breaks = c("Edge Intensity\nRating", 
                                "Ball Carrier\nDirection Change", 
                                "Ball Carrier Speed",
